$(document).ready(function(){
    ...
    $(document).on('click','#btn_control_3anos',function(){
        ...
        $('#listados_estu').empty();
        var id_oferta_aca = 1;
        lista_estudiantes_2022(id_oferta_aca);
    });
    ...
    $(document).on('click','#btn_control_septimo',function(){
        $('#txt_gen').show();
        $('#txt_nivel').text('7° Grado de Educación General Básica');
        $('#listados_estu').empty();
        var id_oferta_aca = 10;
         lista_estudiantes_2022(id_oferta_aca);
    });
...
function lista_confirmados_2022(){
...
function lista_estudiantes_2022(id_oferta_aca){
    let id = id_oferta_aca;
    var id_envio = {"id_envio":id}; 
   $.ajax({
      url: '../../backend/listas_estudiantes_2022.php',
      type: 'POST',
      data: id_envio,   
      success: function (respuesta)
      {
         ...
         listas.forEach(lista =>{
            ...
            <tr elmentoid="${lista.id_usuario}">
               ...
               <td>${lista.Email}</td>
               <td colspan="2">
				    <a href="#"><i class="fas fa-id-card" id="op_estu"></i></a>
                    <a href="#"><i class="fas fa-key" id="key_user"></i></a> 
               </td>
...
$(document).on('click','#key_user',function(){
        $('#capa_1').show();
        let elemento = $(this)[0].parentElement.parentElement.parentElement;
        var id_de_usuario = $(elemento).attr('elmentoid');
        busca_perfil(id_de_usuario);
        /*if(id_de_usuario == 1){
...
$(document).on('click','#btn_asignarperfil', function(){   
    var id_usu_ =$('#Id_usuario').val();
    verficar_perfil(id_usu_);   
    
});    
function verficar_perfil(id_usu_){
    let id = id_usu_;
    var id_envio ={"id_envio":id};
    //alert ('estoy dentro de la funcaion');
        $.ajax({
        ...
        success: function(datos)
        {
            ...
            if (datos==1){
                alert("Este estudiante ya tiene un perfil asignado");
                }            
                else{
                let _formP = $("#form_perfiles");    
                let datos = new FormData($("#form_perfiles")[0]);    
                $.ajax({
...
//***UN SOLO PERFIL-LOGIN ******
function busca_perfil(id_de_usuario){
    let id = id_de_usuario;
    var id_envio ={"id_envio":id}; 
    $.ajax({
        ...
        success: function (respuesta)
            {
                console.log (respuesta);
                const item = JSON.parse(respuesta);
                var nombres_com =  item.Nombres +' '+item.Apellidos;
