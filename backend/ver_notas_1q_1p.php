<?php
    ...
    $consulta = "SELECT Usuarios.Apellidos,Usuarios.Nombres, oferta_academica.nombre_nivel, asignatura.nom_asignatura,notas_1q_1p.nota_cuantitativa, notas_1q_1p.nota_cualitativa FROM `notas_1q_1p` INNER JOIN Usuarios on Usuarios.id_usuario = notas_1q_1p.id_usuario INNER JOIN oferta_academica on oferta_academica.id_oferta_aca = notas_1q_1p.id_oferta_aca INNER JOIN asignatura ON asignatura.id_asignatura = notas_1q_1p.id_asignatura where notas_1q_1p.id_usuario = $id_usuario ORDER BY asignatura.nom_asignatura";
